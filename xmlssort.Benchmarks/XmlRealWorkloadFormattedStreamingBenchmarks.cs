...
[CPUUsageDiagnoser]
public class XmlRealWorkloadFormattedStreamingBenchmarks
{
    ...
    public void GlobalSetup()
    {
        ...
        var sortRules = (Environment.GetEnvironmentVariable(SortRuleEnvironmentVariable) ?? DefaultSortRule).Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        ...
        var parseMethod = commandLineOptionsType.GetMethod("Parse", BindingFlags.Public | BindingFlags.Static) ?? throw new InvalidOperationException("CommandLineOptions.Parse was not found.");
        processMethod = xmlStreamingProcessorType.GetMethod("Process", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static, binder: null, types: [typeof(string), typeof(string), commandLineOptionsType], modifiers: null) ?? throw new InvalidOperationException("XmlStreamingProcessor.Process was not found.");
        options = parseMethod.Invoke(null, [BuildArguments(sortRules)]) ?? throw new InvalidOperationException("CommandLineOptions.Parse returned null.");
    }
    ...
    [Benchmark]
    public void ProcessRealWorkloadWithFormatXml()
    {
    ...
    {
        var arguments = new List<string>((sortRules.Count * 2) + 1);
